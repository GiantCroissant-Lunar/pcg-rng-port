...
internal struct Pcg32K2Engine : IEquatable<Pcg32K2Engine>
{
    private Pcg32Engine _base;
    private readonly uint[] _data;

    // Parameters for pcg32_k2 = ext_setseq_xsh_rr_64_32<1,16,true>
    private const int TablePow2 = 1;          // table size = 2
    private const int AdvancePow2 = 16;       // table advance granularity
    private const ulong TableMask = (1UL << TablePow2) - 1UL; // 1
    private const ulong TickMask = (1UL << AdvancePow2) - 1UL; // 0xFFFF

    public Pcg32K2Engine(ulong seed, ulong stream)
    {
    ...
    private void AdvanceTableSingleTick()
    {
        ...
        if (carry)
        {
            carry = InsideOutRxsMXs32.ExternalStep(ref _data[0], 1UL);
        }
        bool carry2 = InsideOutRxsMXs32.ExternalStep(ref _data[0], 1UL);
        carry = carry || carry2;

        if (carry)
        {
            carry = InsideOutRxsMXs32.ExternalStep(ref _data[1], 2UL);
        }
        carry2 = InsideOutRxsMXs32.ExternalStep(ref _data[1], 2UL);
        carry = carry || carry2;
    }
    ...
    private void AdvanceTable(ulong delta, bool forwards)
    {
        ...
        if (InsideOutRxsMXs32.ExternalAdvance(ref _data[0], 1UL, truncDelta, forwards))
        {
            carry += 1UL;
        }
        ...
        truncDelta = (uint)totalDelta;
        carry = totalDelta >> 32; // basebits (64) > extbits (32)

        if (InsideOutRxsMXs32.ExternalAdvance(ref _data[1], 2UL, truncDelta, forwards))
        {
            carry += 1UL;
        }
    }
    ...
    public bool Equals(Pcg32K2Engine other)
    {
    ...
    {
        return obj is Pcg32K2Engine other && Equals(other);
    }
